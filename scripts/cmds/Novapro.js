...
module.exports = {
  config: {
    name: "nova_pro",
    aliases: ["novapro", "nova", "pro1"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with Nova Pro 1.0",
    longDescription: "Conversational AI powered by Nova Pro 1.0 model.",
    category: "AI",
  ...
  async callAI(uid, msg, envGlobal) {
    ...
    const apiUrl = `${baseUrl}/api/Nova-Pro-1-0?uid=${uid}&msg=${encodeURIComponent(msg)}`;
  ...
  onStart: async function ({ api, event, args, envGlobal }) {
    ...
    try {
      ...
      const finalResponse = `Nova Pro 1.0\n\n${data.reply}`;
      ...
      api.unsendMessage(thinking.messageID);
      api.sendMessage("Nova Pro 1.0 service is currently unavailable.", event.threadID);
    }
  },
