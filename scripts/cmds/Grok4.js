...
module.exports = {
  config: {
    name: "grok_4",
    aliases: ["grok4", "grok", "g4fr"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with Grok 4 Fast Reasoning",
    longDescription: "Conversational AI powered by Grok 4 Fast Reasoning model.",
    category: "AI",
  ...
  async callAI(uid, msg, envGlobal) {
    ...
    const apiUrl = `${baseUrl}/api/Grok-4-Fast-Reasoning?uid=${uid}&msg=${encodeURIComponent(msg)}`;
  ...
  onStart: async function ({ api, event, args, envGlobal }) {
    ...
    try {
      ...
      const finalResponse = `Grok 4 Fast Reasoning\n\n${data.reply}`;
      ...
      api.unsendMessage(thinking.messageID);
      api.sendMessage("Grok 4 Fast Reasoning service is currently unavailable.", event.threadID);
    }
  },
