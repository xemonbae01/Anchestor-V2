...
module.exports = {
  config: {
    name: "gemini25pro",
    aliases: ["g25pro", "gemini2.5pro", "g2.5pro"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with Gemini 2.5 Pro",
    longDescription: "Conversational AI powered by Gemini 2.5 Pro model.",
    category: "AI",
  ...
  async callAI(uid, msg, envGlobal) {
    try {
      ...
      const apiUrl = `${baseUrl}/api/gemini2-5pro?uid=${uid}&msg=${encodeURIComponent(msg)}`;
      const response = await axios.get(apiUrl, { timeout: 20000 });
      return response.data;
    } catch (err) {
      return { status: "error", reply: null };
    }
  },
  ...
  onStart: async function ({ api, event, args, envGlobal }) {
    ...
    try {
      ...
      if (data.status !== "success" || !data.reply) {
        return api.sendMessage("⚠️ AI returned an invalid response.", event.threadID);
      }

      const finalResponse = ` Gemini 2.5 Pro\n\n${data.reply}`;
      ...
      api.unsendMessage(thinking.messageID);
      api.sendMessage("❌ Gemini 2.5 Pro service is currently unavailable.", event.threadID);
    }
  },
