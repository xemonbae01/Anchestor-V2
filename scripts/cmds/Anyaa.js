...
module.exports = {
  config: {
    name: "anyaa",
    aliases: ["anyaai", "forgeranya"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with Anya AI",
    longDescription: "Interact with the cute and curious Anya Forger AI from Redwan's Core.",
    category: "AI",
  ...
  onStart: async function ({ api, event, args }) {
    ...
    const apiUrl = `https://xemo.up.railway.app/api/anya?uid=${uid}&msg=${encodeURIComponent(query)}`;
  ...
  onReply: async function ({ api, event }) {
    try {
      ...
      const uid = event.senderID;
      const apiUrl = `https://xemo.up.railway.app/api/anya?uid=${uid}&msg=${encodeURIComponent(userAnswer)}`;
