...
module.exports = {
  config: {
    name: "gptmini",
    aliases: ["o4mini", "mini", "gpt_o4_mini"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with GPT O4 Mini",
    longDescription: "Conversational AI powered by GPT O4 Mini model.",
    category: "AI",
  ...
  async callAI(uid, msg, envGlobal) {
    ...
    const apiUrl = `${baseUrl}/api/gpt_o4_mini?uid=${uid}&msg=${encodeURIComponent(msg)}`;
  ...
  onStart: async function ({ api, event, args, envGlobal }) {
    ...
    try {
      ...
      const finalResponse = `GPT O4 Mini\n\n${data.reply}`;
      ...
      api.unsendMessage(thinking.messageID);
      api.sendMessage("GPT O4 Mini service is currently unavailable.", event.threadID);
    }
  },
