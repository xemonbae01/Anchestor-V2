...
module.exports = {
  config: {
    name: "minimax_m1",
    aliases: ["minimax", "m1", "minim"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with MiniMax M1",
    longDescription: "Conversational AI powered by MiniMax M1 model.",
    category: "AI",
  ...
  async callAI(uid, msg, envGlobal) {
    ...
    const apiUrl = `${baseUrl}/api/MiniMax-M1?uid=${uid}&msg=${encodeURIComponent(msg)}`;
  ...
  onStart: async function ({ api, event, args, envGlobal }) {
    ...
    try {
      ...
      const finalResponse = `MiniMax M1\n\n${data.reply}`;
      ...
      api.unsendMessage(thinking.messageID);
      api.sendMessage("MiniMax M1 service is currently unavailable.", event.threadID);
    }
  },
