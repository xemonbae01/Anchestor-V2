...
module.exports = {
  config: {
    ...
    countDown: 3,
    role: 0,
    shortDescription: "Chat with Ben 10 AI",
    longDescription: "Talk with Ben Tennyson, the wielder of the Omnitrix, powered by Redwan's Core AI.",
    category: "AI",
  ...
  onStart: async function ({ api, event, args }) {
    ...
    const apiUrl = `https://xemo.up.railway.app/api/ben?uid=${uid}&msg=${encodeURIComponent(query)}`;
    ...
    try {
      ...
      const finalResponse = `🟢 𝘽𝙀𝙉 𝙏𝙀𝙉 𝘼𝙄 🟢\n\n${data.reply}`;
      ...
      console.error(error);
      api.sendMessage("❌ Ben 10 AI service is currently unavailable!", event.threadID, event.messageID);
    }
  },
  ...
  onReply: async function ({ api, event }) {
    try {
      ...
      const uid = event.senderID;
      const apiUrl = `https://xemo.up.railway.app/api/ben?uid=${uid}&msg=${encodeURIComponent(userAnswer)}`;
