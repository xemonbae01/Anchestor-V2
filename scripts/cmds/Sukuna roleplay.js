...
module.exports = {
  config: {
    name: "sukuna",
    aliases: ["ryomen", "sukunai", "sukunaai"],
    version: "1.0",
    author: "Redwan (Xe Mo)",
    countDown: 3,
    role: 0,
    shortDescription: "Chat with Sukuna AI",
    longDescription: "Talk with Ryomen Sukuna, the King of Curses from Jujutsu Kaisen, powered by Redwan's Core AI.",
    category: "AI",
  ...
  onStart: async function ({ api, event, args }) {
    ...
    const apiUrl = `https://xemo.up.railway.app/api/sukuna?uid=${uid}&msg=${encodeURIComponent(query)}`;
  ...
  onReply: async function ({ api, event }) {
    try {
      ...
      const uid = event.senderID;
      const apiUrl = `https://xemo.up.railway.app/api/sukuna?uid=${uid}&msg=${encodeURIComponent(userAnswer)}`;
